...
START_TEST(test_es_length)
{
	unsigned int ret;
        ...
        makebuf(buf, test1, sizeof(buf));
	ret = es_length(buf);
	fail_unless(ret == 14, "[Task 2.9.b] Wrong length with string \"%s\". Got %d, expected 14\n",
                test1, ret);

        char *test2 = "#viisibazfoo";
        makebuf(buf, test2, sizeof(buf));
	ret = es_length(buf);
	fail_unless(ret == 0, "[Task 2.9.b] Wrong length with string \"%s\". Got %d, expected 0\n",
                test2, ret);
}
END_TEST
...
START_TEST(test_es_copy)
{
	...
	fail_unless(ret == 15, "[Task 2.9.c] Wrong return value with string \"%s\". Got %d, expected 15\n",
                test1, ret);
        //char infostr[100] = "";
        ...
        buf[strlen(ref1)] = 0;
        remove_nonascii(buf);
	fail_unless(!ret, "[Task 2.9.c] Copied string is incorrect with input \"%s\". Should have been \"%s\", you have \"%s\"",
                test1, ref1, buf);

        //infostr[0] = 0;
        char *test2 = "#viisibazfoo";
	ret = es_copy(buf, test2);
	fail_unless(ret == 0, "[Task 2.9.c] Wrong return value with string \"%s\". Got %d, expected 0\n",
                test2, ret);
        char *ref2 = "#";
...
START_TEST(test_es_token)
{
	...
	char *ret;
        ...
        strcpy(orig2, str2);
	ret = es_token(str1, ':');
	fail_unless(ret == NULL, "[Task 2.9.d] With string \"%s\" and token \':\' should have returned NULL. Remember that the end of the string here is marked with \'#\'",
                orig1);
        //char infostr[100] = "";
	...
	fail_unless(!ret2, "[Task 2.9.d] Processed string is incorrect with input (\"%s\", ':'). Should have been \"%s\", you have \"%s\"",
                orig1, ref1, str1);

	ret = es_token(str2, ':');
	fail_unless(ret == &(str2[7]), "[Task 2.9.d] Wrong return value with string \"%s\": Got %p, expected %p\n",
                orig2, ret, &(str2[7]));
        //infostr[0] = 0;
...
{
	Suite *s = suite_create("Test-2.9");

	tmc_register_test(s, test_es_print, "2.9.a");
	tmc_register_test(s, test_es_length, "2.9.b");
	tmc_register_test(s, test_es_copy, "2.9.c");
	tmc_register_test(s, test_es_token, "2.9.d");
