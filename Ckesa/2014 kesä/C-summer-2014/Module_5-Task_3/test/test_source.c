...
{   
    ...
    for (i = 0; i < 6; i++) {
        ...
        fail_unless(ret == results[i], "[Task 5.3.a] Should have returned %d for file %s. You returned %d.", results[i], files[i], ret);
    }
}
END_TEST
...
START_TEST(test_word_count)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        fail_unless(ret == results[i], "[Task 5.3.b] Should have returned %d for file %s. You returned %d.", results[i], files[i], ret);
    }
}
END_TEST
...
START_TEST(test_most_common)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        fail_unless(ret == results[i],
                "[Task 5.3.c] Should have returned %s for file %s. You returned %s.",
                buf, files[i], buf2);
    }
}
END_TEST
...
int main(int argc, const char *argv[])
{
    ...
    tmc_register_test(s, test_line_count, "5.3.a");
    tmc_register_test(s, test_word_count, "5.3.b");
    tmc_register_test(s, test_most_common, "5.3.c");
