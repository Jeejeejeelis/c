...
START_TEST(test_set)
{
    ...
    if (!checkAD(buffer, &ad, &ref)) {
        fail("[Task 4.1.a] After setByte(%d), %s", v1, buffer);
    }
    ...
    if (!checkAD(buffer, &ad, &ref)) {
        fail("[Task 4.1.a] After setInt(%d), %s", v2, buffer);
    }
        
    ad = setDouble(5.555);
    ref.type = DOUBLE; ref.value.dval = 5.555;
    if (!checkAD(buffer, &ad, &ref)) {
        fail("[Task 4.1.a] After setDouble(%lf), %s", 5.555, buffer);
    }
}
END_TEST
...
START_TEST(test_print)
{
    ...
    if (strcmp(buffer, "B:4")) {
        ...
        fail("[Task 4.1.b] After setByte(4) should output '%s', you printed: '%s'",
                "B:4", buffer);
    }
    ...
    if (strcmp(buffer, "I:20000")) {
        remove_nonascii(buffer);
        fail("[Task 4.1.b] After setInteger(20000) should output '%s', you printed: '%s'",
                "I:20000", buffer);
    }
    ...
    if (strcmp(buffer, "D:6.666600")) {
        remove_nonascii(buffer);
        fail("[Task 4.1.b] After setInteger(6.6666) should output '%s', you printed: '%s'",
                "D:6.666600", buffer);
    }    
}
END_TEST
...
int main(int argc, const char *argv[])
{
	...
	tmc_register_test(s, test_set, "4.1.a");
	tmc_register_test(s, test_print, "4.1.b");
