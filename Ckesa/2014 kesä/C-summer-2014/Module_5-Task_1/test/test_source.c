...
START_TEST(test_hexdump)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        fail_unless(ret == refvals[i], "[Task 5.1.b] hexdump(\"%s\") should return %d. You returned %d.",
                files[i], refvals[i], ret);
        ...
        if (ret != -1 && mycompare(stu, refouts[i], infostr)) {
            remove_nonascii(stu);
            fail("[Task 5.1.b] Called hexdump(\"%s\"). Your output:\n%s\nReference output:\n%s\nReason: %s\n",
                    files[i], stu, refouts[i], infostr);
        }
    }
}
...
int main(int argc, const char *argv[]) {
    ...
    tmc_register_test(s, test_textdump, "5.1.a");
    tmc_register_test(s, test_hexdump, "5.1.b");    
