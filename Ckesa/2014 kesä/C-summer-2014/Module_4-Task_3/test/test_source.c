...
START_TEST(test_init_array)
{
    char **arr = init_array();
    if (!arr) {
        fail("[Task 4.3.a] init_array returned NULL");
    }
    if (*arr != NULL) {
        fail("[Task 4.3.a] After initialization the first object should be NULL");
    }
    free_strings(arr);
}
END_TEST
...
START_TEST(test_add_string)
{
    ...
    char **arr = init_array();
    if (!arr) {
      fail("[Task 4.3.b] Implement init_array first");
    }
    for (unsigned int i = 0; i < num; i++) {
        char **a2 = add_string(arr, ref[i]);
        if (!a2) {
            free_strings(arr);
            fail("[Task 4.3.b] add_string(ptr, \"%s\") returned NULL",
                    ref[i]);
        }
	arr = a2;
    }
    ...
    if (arr[num] != NULL) {
        free_strings(arr);
        fail("[Task 4.3.b] After adding %d strings, array member %d should be NULL",
                num, num);
    }
    ...
    for (unsigned int i = 0; i < num; i++) {
        if (arr[i] == NULL) {
            free_strings(arr);
            fail("[Task 4.3.b] After adding %d strings, array member %d is NULL, but it should point to string",
                    num, i);
        }
        if (strcmp(arr[i], ref[i])) {
            remove_nonascii(arr[i]);
            char buf2[200];
            sprintf(buf2, "[Task 4.3.b] After adding strings %s, array member %d is %s, should be %s",
                    buf, i, arr[i], ref[i]);
            free_strings(arr);
            fail(buf2);
        }
